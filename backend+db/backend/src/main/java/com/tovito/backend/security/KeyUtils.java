...
public class KeyUtils {
    ...
    private KeyPair getKeyPair(String publicKeyPath, String privateKeyPath) {
        ...
        //если директории существуют, то открываем их и считываем пару ключей
        if (publicKeyFile.exists() && privateKeyFile.exists()) {
            log.info("loading keys from file: {}, {}", publicKeyPath, privateKeyPath);
            try {
                KeyFactory keyFactory = KeyFactory.getInstance("RSA"); //алгоритм для конвертации данных в ключ, в данном случае РСА
            ...
            //короче если в продакшене делаю, то подгружаю ключи вручную из пропертизов
